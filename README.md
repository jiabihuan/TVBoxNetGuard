...
ACK 也被计进配额 → 配额被数据大包吃光 → ACK 发不出 → 对端停止下发 →
表现就是"一开引擎整台盒子断网"。root 流量闸（uid 0 限 128 KB/s ≈ 87 pps）
踩得最狠：系统服务、DNS、内核发出的 ACK 全是 uid 0，于是**整机连不上网**。

现在所有限速路径都会**放行小包**（IP 层 ≤ 200 字节：纯 ACK 40~60 字节、
DNS 查询通常 < 128 字节），只对大块数据限速：

| 路径 | 做法 |
| --- | --- |
| iptables | 限速规则前插 `-m length --length 0:200 -j ACCEPT` |
| tc | mangle 里只对 `-m length --length 400:65535` 的包做 `CLASSIFY` |
| VPN 引擎 | `Limiter` 中 ≤ 128 字节的包不扣令牌桶 |

`blocked`（彻底断网）和「上行 0」不受豁免影响，该封的照样封死。

> **root 流量闸默认关闭**。它最容易踩到 ACK 饿死，现在默认关闭，需要时
> 在设置页手动开（开启后也已豁免小包）；若打开后整机上网异常，关掉即可。

**实测验证**（veth + 双 netns，不同 uid 的进程模拟不同 App，脚本见 `labtest/`）：

| 目标：把某应用上行压到 200 KB/s | 该应用上行 | 普通应用 | root/系统 |
| --- | --- | --- | --- |
| 基线（无规则） | 78 MB/s | 78 MB/s | 89 MB/s |
| **限速 200 KB/s** | **186 KB/s**（限住） | 226 MB/s（不误伤） | 170 MB/s（不误伤） |
| **禁 UDP** | **0**（PCDN 掐死） | 192 MB/s | 188 MB/s |

对照实验：去掉小包豁免后 root 的下行从 4.6 GB/s 暴跌到 308 MB/s（**-93%**，
即用户遇到的"整机断网"），加上豁免后恢复到 5.06 GB/s。

### 1.2 限速方案选型（实测）

| 方案（目标 200 KB/s） | 实测 | 结论 |
| --- | --- | --- |
| `tc + htb` | 190 KB/s | ✅ 最精确，优先使用 |
| `iptables -m limit` | 172 KB/s | ✅ 可靠，tc 不可用时的兜底 |
| `iptables -m statistic nth` | 38 MB/s | ❌ 已弃用：比例丢弃的实际限速值随应用原始速率剧烈变化，不可控 |

tc 路径还带一道安全阀：校验 htb 的 `default class` 是否真的建成，
建不起来就回滚 qdisc 并回退 iptables，避免"未分类流量被丢弃 = 整机断网"。
